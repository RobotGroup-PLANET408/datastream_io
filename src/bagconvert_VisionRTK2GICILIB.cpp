...
int main(int argc, char **argv)
{
    ...
    std::string bag_outfilepath = config.Output_Filepath + "/GICILIB.bag";
    ...
    if (topics_output.find(config.GNSSRoveObs_Topic_Output) == topics_output.end() && topics_output.find(config.GNSSRoveNav_Topic_Output) == topics_output.end())
    {
        ...
        for (int i = 0; i < config.rosbag_filename.size(); i++)
        {
            ...
            if (bag_infilepath != "\0")
            {
                ...
                {
                    ROS_ERROR("Fail to extract GNSS raw data from bag file.");
                    return false;
                }

                // step 2: convert GNSS observation data format
                std::list<datastreamio::GICILIB_GnssObservations> gici_obsdata(0);
                if (dataio_common::Convert_GNSSObsData_IPS2OtherFormat(ips_obsdata, gici_obsdata, config.GNSSRoveObs_DataFormat_Output) == false)
                {
                ...
                if (dataio_common::Convert_GNSSEphData_IPS2OtherFormat(ips_ephdata, gici_ephdata, config.GNSSRoveNav_DataFormat_Output) == false)
                {
                ...
                for (double timestamp = start_time.toSec(); timestamp <= end_time.toSec(); timestamp += 30.0)
                {
                    ...
                    dataio_common::Bind_GNSSEphData_Single2Set(gici_ephdata, one_set, gps_timestamp, config.GNSSRoveNav_DataFormat_Output);
                ...
                if (dataio_common::Write_ROSMessage_ROSBag(outfile_bag, config.GNSSRoveObs_Topic_Output, gici_obsdata, start_time, end_time) == false)
                {
    ...
    if (config.GNSSBaseObs_FilePath_Input != "\0" && topics_output.find(config.GNSSBaseObs_Topic_Output) == topics_output.end())
    {
        ...
        std::list<datastreamio::GICILIB_GnssObservations> gici_baseobs(0);
        if (dataio_common::Convert_GNSSObsData_IPS2OtherFormat(ips_baseobs, gici_baseobs, config.GNSSBaseObs_DataFormat_Output) == false)
        {
        ...
        if (dataio_common::Write_ROSMessage_ROSBag(outfile_bag, config.GNSSBaseObs_Topic_Output, gici_baseobs, start_time, end_time) == false)
        {
    ...
    if (config.GNSSBaseNav_FilePath_Input != "\0" && topics_output.find(config.GNSSBaseNav_Topic_Output) == topics_output.end())
    {
        ...
        if (dataio_common::Convert_GNSSEphData_IPS2OtherFormat(ips_baseeph, gici_eph, config.GNSSBaseNav_DataFormat_Output) == false)
        {
        ...
        for (double timestamp = start_time.toSec(); timestamp <= end_time.toSec(); timestamp += 30.0)
        {
            ...
            /// NOTE: use GPS time to search and bind
            double gps_timestamp = timestamp - GPS_LINUX_TIME + LEAP_SECOND;
            dataio_common::Bind_GNSSEphData_Single2Set(gici_eph, gici_ephs, gps_timestamp, config.GNSSBaseNav_DataFormat_Output);
            ...
            gici_navdata.push_back(onemsg);
        }

        // step 4: write GNSS navigation data to bag file
        if (dataio_common::Write_ROSMessage_ROSBag(outfile_bag, config.GNSSBaseNav_Topic_Output, gici_navdata, start_time, end_time) == false)
        {
    ...
    std::string basepos_topic = "/gici/gnss_reference/antenna_position";
    if (topics_output.find(basepos_topic.c_str()) == topics_output.end())
    {
        ...
        for (double timestamp = start_time.toSec(); timestamp <= end_time.toSec(); timestamp += deltaT)
        {
            ...
            outfile_bag.write(basepos_topic, start_time, gnss_antpos);
        }
