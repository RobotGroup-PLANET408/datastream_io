...
int main(int argc, char **argv)
{
    ...
    {
        // Get filepath
        std::string bag_outfilepath = config.output_filepath + "/Intel_Convert_" + std::to_string(i) + ".bag";
        ...
        if (config.gnssroveobs_filepath != "\0" && topics_output.find(config.image_topic_output) == topics_output.end())
        {
            ROS_INFO("  processing gnss raw data ...");

            // step 1: extract GNSS observation data
            std::list<gnss_common::IPS_OBSDATA> ips_baseobs(0);
            size_t dotpos = config.gnssroveobs_filepath.find_last_of(".");
            if (dotpos != std::string::npos && dotpos < config.gnssroveobs_filepath.length())
            {
                std::string filetype = config.gnssroveobs_filepath.substr(dotpos + 1);
                if (dataio_common::Extract_GNSSObsData_TXTFile_MAIN(config.gnssroveobs_filepath.c_str(), filetype, ips_baseobs) == false)
                {
            ...
            if (dataio_common::Write_ROSMessage_ROSBag(bag_outfilepath.c_str(), config.gnssroveobs_topic_output, config.imu_topic_output, robot_baseobs, 2, true) == false)
            {
