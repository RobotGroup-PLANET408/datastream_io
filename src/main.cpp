...
///< Extract and write IntelD457 raw data
void Extract_and_Write_IntelD457(int argc, char **argv);

///< Receive real-time data to process
void RTIEnode_Driver(int argc, char **argv);
...
int main(int argc, char **argv)
{
    ...
    ///< TODO: Extract and write Intel D457 raw data
    // Extract_and_Write_IntelD457(argc, argv);

    ///< TODO: Extract and write VisionRTK raw data
    Extract_and_Write_VisionRTK(argc, argv);

    ///< TODO: Receive real-time data to process
    // RTIEnode_Driver(argc, argv);
...
{
    // Launch the ROS node
    ros::init(argc, argv, "datastreamio_node");

    // Receive, convert and publish GNSS base station
    // NOTE: The GNSS base data should include observation and ephemeris
    // NOTE: The GNSS base data should be ros message
    // NOTE: [8.148.22.229: 3002] FOR base station

    // std::thread baseThread([&]()
    //                        { dataio_common::Receive_and_Publish_GNSSData_BaseStation(NULL); });
    ...
    // Receive, convert and publish GNSS rove station
    // NOTE: The GNSS base data should be ros message
    // NOTE: [10.0.1.1: 20010] or [10.0.2.1: 20010] FOR rove station

    // std::thread roveThread([&]()
    //                        { dataio_common::Receive_and_Publish_GNSSData_RoveStation(NULL); });
...
void Extract_and_Write_VisionRTK(int argc, char **argv)
{
    ...
    if (bag_infilepath != "\0")
    {
        ...
        dataio_common::Extract_GNSSSolution_ROSBag(bag_infilepath.c_str(), sol_datas, dataio_common::VisionRTK2_gnsssol_topic, dataio_common::dataformat::VisionRTK_format_02, dataio_common::timesystem::Linux_time);
        // step 2: convert GNSS solution data
        dataio_common::Convert_GNSSSolution(sol_datas, dataio_common::dataformat::VisionRTK_format_02, dataio_common::dataformat::RobotGVINS_format);
    ...
    if (bag_infilepath != "\0")
    {
        ...
        dataio_common::Extract_GNSSRawData(bag_infilepath.c_str(), dataio_common::VisionRTK2_gnssraw_topic, dataio_common::dataformat::VisionRTK_format_02, dataio_common::timesystem::Linux_time, ips_obsdata, ips_ephdata);
...
void Extract_and_Write_IntelD457(int argc, char **argv)
{
    ...
    if (bag_infilepath != "\0")
    {
        // step 1: extract imu data from bag file
        std::list<sensor_msgs::Imu> imudatas(0);
        dataio_common::Extract_IMUdata_ROSBag(bag_infilepath.c_str(), dataio_common::IntelD457_imu_topic, imudatas, dataio_common::Linux_time);
        // step 2: write imu data to bag file
        dataio_common::Write_IMUdata_ROSBag(bag_outfilepath.c_str(), dataio_common::RobotGVINS_imu_topic, imudatas, 1);
    }
    ...
    if (bag_infilepath != "\0")
    {
        // step 1: extract image data from bag file
        std::list<sensor_msgs::Image> imagedatas(0);
        dataio_common::Extract_CompressedImageData_ROSBag(bag_infilepath.c_str(), dataio_common::IntelD457_compressimage_topic, imagedatas, dataio_common::Linux_time);
        // step 2: write image data to bag file
        dataio_common::Write_ImageData_ROSBag(bag_outfilepath.c_str(), dataio_common::RobotGVINS_image_topic, imagedatas, 2);
    }
